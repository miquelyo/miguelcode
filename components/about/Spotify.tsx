...
export default function Spotify() {
  ...
  if (isLoading) {
    return (
      <div className={`inline-flex min-w-[250px] rounded-xl bg-zinc-200 p-3 pr-3 dark:bg-zinc-800 md:pr-10`}>
  ...
  if (error || !data) {
    return (
      <div className="inline-flex min-w-[250px] max-w-full items-center rounded-xl bg-zinc-200 p-3 dark:bg-zinc-800">
        <div className="w-[75px] overflow-hidden rounded-lg p-5 sm:w-[100px]">
          <LogoSpotify className="h-8 w-8" />
        </div>
        <div className="mx-5">
          <h3 className="mb-1 text-xs font-semibold sm:text-sm">SPOTIFY</h3>
          <p className="text-sm font-medium text-zinc-600 dark:text-zinc-300 sm:text-base">Tidak ada data Spotify saat ini.</p>
        </div>
      </div>
    )
  }
